        ...
        <div class="row">
                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item"><a href="{{route('manager.products.index')}}">{{__('manager.product')}}</a></li>
                            <li class="breadcrumb-item active">{{__('manager.image')}}</li>
                        </ol>
                    </div>
                    <h4 class="page-title">{{__('manager.edit_product_images')}}</h4>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="header-title">{{__('manager.product_images')}}</h4>
                            <form action="{{route('manager.product-images.store',['id'=>$productId])}}" method="post" class="dropzone" id="myAwesomeDropzone">
                                @csrf
...
@section('script')
    <script>
        ...
        function removeImage(file){
            $.ajax({
                url: {!! json_encode(route('manager.product-images.delete'))!!},
                type:"DELETE",
                data:{
                    id:file.id,
